<?php
add_action('widgets_init', 'imenso_footer_widget_contact');
function imenso_footer_widget_contact()
{
    return register_widget('imenso_footer_contact_widget');
}

class imenso_footer_contact_widget extends WP_Widget
{

    function __construct()
    {
        parent::__construct(
            'imenso_footer_contact_widget', // Base ID
            __('imenso Footer Contact', 'imenso'), // Name
            array('description' => __('Your contact details', 'imenso'),) // Args
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        <address>
            <p><i class="fa fa-map-marker"></i> <?php if ($Contact_address) {
                    ...
                    echo _('25, Lorem Lis Street', 'imenso');
                } ?></p>
    ...
    public function form($instance)
    {
        if (isset($instance['title'])) {
            ...
            $title = __('Contact Info', 'imenso');
        }

        if (isset($instance['Contact_phone_number'])) {
            $Contact_phone_number = $instance['Contact_phone_number'];
        } else {
            $Contact_phone_number = __('0764-989879', 'imenso');
        }

        if (isset($instance['Contact_email_address'])) {
            $Contact_email_address = $instance['Contact_email_address'];
        } else {
            $Contact_email_address = __('contact@me.com ', 'imenso');
        }

        if (isset($instance['website_add'])) {
            $website_add = $instance['website_add'];
        } else {
            $website_add = __('http://www.webhuntinfotech.com', 'imenso');
        }

        if (isset($instance['Contact_address'])) {
            $Contact_address = $instance['Contact_address'];
        } else {
            $Contact_address = __('NewYork', 'imenso');
        }

        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'imenso'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>"
                   name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text"
                   value="<?php echo esc_attr($title); ?>"/>
        </p>
        <p><label
                for="<?php echo esc_attr($this->get_field_id('Contact_phone_number')); ?>"><?php _e('Contact phone number:', 'imenso'); ?></label>
        ...
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('Contact_email_address')); ?>"><?php _e('E-mail address:', 'imenso'); ?></label>
        ...
        <p><label
                for="<?php echo esc_attr($this->get_field_id('website_add')); ?>"><?php _e('Website :', 'imenso'); ?></label>
        ...
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('Contact_address')); ?>"><?php _e('Contact address:', 'imenso'); ?></label>
