<?php
add_action('widgets_init', 'imenso_recent_posts_widgets');
function imenso_recent_posts_widgets()
{
    return register_widget('imenso_footer_recent_posts');
}

/**
 * Adds widget for recent Post in footer.
 */
class imenso_footer_recent_posts extends WP_Widget
{
    function __construct()
    {
        parent::__construct(
            'imenso_footer_recent_posts', //ID
            __('imenso Recent Posts', 'imenso'), // Name
            array('description' => __('Display Recent posts on your sites', 'imenso'),) // Args
        );
    }
            ...
            <?php while ($loop->have_posts()) : $loop->the_post(); ?>
                <li class="clearfix">
                    <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
                        <span class="recent_posts_img"><?php the_post_thumbnail('imenso_recent_widget_thumb'); ?></span>
    ...
    public function form($instance)
    {
        ...
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('number_of_posts')); ?>"><?php _e('Number of pages to show:', 'imenso'); ?></label>
