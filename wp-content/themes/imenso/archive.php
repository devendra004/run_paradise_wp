...
?>
    <section class="content_section page_title">
        <div class="content clearfix">
            <h1 class="">
                <?php if (is_day()):
                    printf(__('Daily Archives: %s', 'imenso'), '<span>' . get_the_date() . '</span>');
                elseif (is_month()) :
                    printf(__('Monthly Archives: %s', 'imenso'), '<span>' . get_the_date(_x('F Y', 'monthly archives date format', 'imenso')) . '</span>');
                elseif (is_year()) :
                    printf(__('Yearly Archives: %s', 'imenso'), '<span>' . get_the_date(_x('Y', 'yearly archives date format', 'imenso')) . '</span>');
                else :
                    _e('Archives', 'imenso');
                endif; ?>
            </h1>
            <?php imenso_breadcrumbs(); ?>
        </div>
    </section>
    <!-- Our Blog Grids -->
    ...
    <section class="content_section">
        <div class="content row_spacer">
            <div class="main_title centered upper">
                <h2><span class="line"><i class="fa fa-archive"></i></span><?php echo esc_attr_e('Archives', 'imenso'); ?>
                </h2>
            </div>
            <?php
            $blog_layout = $imenso_theme_options['blog_layout'];
            $imageSize = $blog_layout == "blogfull" ? 'imenso_home_post_full_thumb' : 'imenso_home_post_thumb';
